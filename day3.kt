...
data class Inst(val dir: Dir, val dist: Int)

fun runner(d: String): Pair<Int, Int> {
    val input = d.split("\n")
            .filter { it.isNotBlank() }
            .map<String, List<Inst>> { wire ->
                val wirePath = wire.split(',')
                        .filter { it.isNotBlank() }
                        .map<String, Inst> {
                            val dd = it.get(0)
                            val ddd = when (dd) {
                                ...
                                'R' -> Dir.RIGHT
                                'L' -> Dir.LEFT
                                else -> throw IllegalArgumentException("unexpected direction: $d")
                            }
    ...
    return Pair(taskOne(input), taskTwo(input))
}

fun taskOne(data: List<List<Inst>>): Int {
    val pts: List<Set<Point>> = data.map<List<Inst>, Set<Point>>( { wire ->
        Runner().go(wire)
    })
    val commonPoints = pts[0].intersect(pts[1])
    return commonPoints.map { it.first.absoluteValue + it.second.absoluteValue } .min() ?: 0

}

fun taskTwo(data: List<List<Inst>>): Int {
    val pts: List<Map<Point, Int>> = data.map { wire ->
        Runner2().go(wire)
    }
    val red = pts[0]
    val yellow = pts[1]
    val commonPoints = red.keys.intersect(yellow.keys)
    return commonPoints.map { red[it]!! + yellow[it]!! } .filterNotNull().min()!!
}
...
class Runner(var ptr: Point = 0 to 0, var counter: Int = 0) {
    public fun go(code: List<Inst>): Set<Point> {
        var pts = mutableSetOf<Point>()
        code.forEach { (dir, dist) ->
            val vector = when (dir) {
                Dir.UP -> 1 to 0
                Dir.DOWN -> -1 to 0
                Dir.LEFT -> 0 to 1
...
class Runner2(var ptr: Point = 0 to 0, var counter: Int =0) {
    public fun go(code: List<Inst>): Map<Point, Int> {
        var pts = mutableMapOf<Point, Int>()
        code.forEach { (dir, dist) ->
            val vector = when (dir) {
                Dir.UP -> 1 to 0
                Dir.DOWN -> -1 to 0
